function crowd_plot(set_list,dir_ext,seting_class,dateset_class)
...
%for i = 1:size(set_list,1)
    ...
    for j = 1:model_num
        load(sprintf('%s/%s/crowd_pr_info_%s_%s.mat',dir_ext, model_name{j}, model_name{j}, set_list));
    ...
    plot_pr(propose, recall, name_list, seting_class, set_list,dateset_class);
%end
