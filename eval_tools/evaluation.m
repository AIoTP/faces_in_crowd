function evaluation(pre_list,gt_dir,setting_name,setting_class,legend_name)
load(gt_dir);
...
for i = 1:3 
    gt_bbx_list = face_box_list{i};
    pred_list = pre_list{i};
    img_pr_info_list = cell(5,1);
    evali_list=eval_list{i};
    
    fprintf('%s, current data set %d\n',setting_name,i);
    for j =1:5
        jj=evali_list(j);
        gt_bbx = gt_bbx_list{jj};
        pred_info = pred_list{j};
        keep_index = (1:length(gt_bbx));
...
save(sprintf('./plot/baselines/Val/%s/%s/crowd_pr_info_%s_%s.mat',setting_class,legend_name,legend_name,setting_name),'pr_cruve','legend_name','-v7.3');
end
